...
├── build-templates/      # (generated by Nx for Azure Functions code generation)
└── tsconfig.base.json    # shared TypeScript configuration

libs/
└── monecromanci-v2/      # the actual CLI implementation (delegated to by @mnci/cli)

tools/
├── python-version-actions.js   # Nx VersionActions implementation for Python packages
└── (other tooling as needed)
...
- **Build**: esbuild (Node apps), Rollup (npm libs), `python -m build` (Python), `go build` (Go)
...
- **`packages/cli/src/commands/add/go.ts`** — the four Go kinds, delegating to
  ...
  Flutter was investigated and deferred — `@nxrocks/nx-flutter` cannot load on
  Nx 23 (it imports `@nx/workspace/src/utilities/fileutils`, removed in 23) and
  no alternative plugin exists.
...
- **`e2e/boxout.e2e.mjs`** — real generation → lint/test/build/package for all kinds (JS & Python)
...
```bash
npm run build          # build @mnci/cli, @mnci/nx-python-pip, monecromanci-v2
npm run test           # unit tests (cli, nx-python-pip)
...
- `e2e/boxout.e2e.mjs` — real `mnci new` + `mnci add` for all kinds
