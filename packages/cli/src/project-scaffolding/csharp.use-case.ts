 ...
 * live in the ASP.NET Core SHARED FRAMEWORK (`Microsoft.AspNetCore.App`),
 * not in any NuGet package. `Microsoft.NET.Sdk.Web`-based projects reference
 * that framework implicitly; `Azure.Functions.Sdk` does not, and neither
 * Functions package pulls it in as a transitive dependency — confirmed
 * against Microsoft's own docs, where the line only turns up in a separate
 * "Performance optimizations" section, framed as a cold-start improvement,
 * while the 4-step "To enable ASP.NET Core integration for HTTP" guide never
 * mentions it. It is both a perf knob and, for this SDK, a compile-time
 * requirement. Adding this line alone was not sufficient by itself — see
