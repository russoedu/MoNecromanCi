...
### 32. A `cli-lib` kind — P2

Filed while building the uniform `build`/`build:dev`/`start`/`dev` script and VS Code
launch-config convention (§4/#33 below): every *app* kind now gets all four, and
every plain library kind deliberately gets none of them — a library has no runtime
process to start, build a debug variant of, or watch. A CLI library sits between the
two: it publishes like an ordinary library (an npm package with a `bin` entry, or the
equivalent in another ecosystem) but is *invoked* like an app, so a user iterating on
it wants exactly what `dev` gives an app — a debug build, watched, re-run on every
change — and a VS Code launch config to set breakpoints in it directly.

**No such kind exists yet.** `add.ts`'s kind list has `npm-lib`/`internal-lib` (plain
libraries, `registerProjectCommands({ build: true })`, no start/dev) and the various
`*-app` kinds (full `build`/`build:dev`/`start`/`dev`) — nothing in between. Forcing a
CLI tool into `npm-lib` today gets it published correctly but with no `dev` loop at
all; forcing it into `node-app` gets the dev loop but the wrong packaging (an `app`
under `apps/`, not a publishable package under `packages/`, and no `bin` field wiring).

**Deliberately not implemented now** — this was raised and explicitly deferred by the
user while scoping the script/workspace-convention task, in favor of finishing that
convention for the kinds that already exist. Left here as the next kind to add, not
folded into `npm-lib` as a flag: a `bin`-entry publishable package with a `dev` target
is a different enough shape (packaging root, entry point, and — per this task's own
convention — a `mnci: <name> dev` launch config) that it reads better as its own kind
than as a conditional branch inside `npm-lib`'s.
...
### 33. A uniform `build`/`build:dev`/`start`/`dev` convention, plus a launch config per app — ✅ done

Every app kind previously had its own ad hoc mix of scripts — some had `start`, none
had a real `dev` loop, and none had a VS Code Run-and-Debug entry, only a Task. Locked
in against an explicit contract given by the user rather than inferred: `build` is a
production build; `build:dev` is a build carrying whatever debug information the
toolchain distinguishes (source maps, unoptimized codegen, debug symbols) — omitted
entirely where a language has nothing to distinguish; `start` runs what `build`
already produced, no rebuild, no watch; `dev` builds a debug version and watches,
re-running/rebuilding on every change. Deliberately scoped to **apps only** — a plain
library has no process to start or watch, so `registerProjectCommands({ build: true })`
alone is unchanged for every library kind. A future `cli-lib` kind (#32 above) would
need the same treatment a library normally skips, explicitly deferred rather than
implemented here.

**Every `dev` target also gets a per-project VS Code launch config**, not just a Task —
the user's own explicit requirement. `vscodeWorkspace()`'s four fixed workspace-level
configs (`mnci: build/test/lint/typecheck`) are matched and regenerated by **exact
name**, not a prefix, which is the bug this shipped with first: a prefix-based filter
(`startsWith('mnci: ')`) would have deleted every per-project `mnci: <name> dev` entry
on the next `mnci upgrade`, since those entries share the same prefix by design.
Mutation-tested: reverting to the prefix filter fails the new "survives an upgrade"
test with the exact expected diff.

**One shared mechanism, six different concrete implementations** — the contract is
uniform, the toolchain underneath it is not:

- **Node**: `@nx/js:node`'s `buildTarget` OPTION does its own project-graph lookup and
  needs the manifest's real scoped name (verified the hard way: `nx run api:build`
  resolves fine via the CLI's own short-name tolerance, but the executor option does
  not), so `nodeAppStartTarget` reads it back off the manifest rather than
  reconstructing it. `start` sets `watch: false` against the generator's own inferred
  `build` target (cached by Nx if unchanged); `build:dev` is the generator's own
  `development` configuration, already source-mapped, just given a name in the
  convention; `dev` is the generator's own `serve` target, untouched. A Node Function
  App's `dev` runs esbuild `--watch` and `func start` as two `nx:run-commands` siblings
  under `parallel: true`, with `dependsOn: ['build:development']` closing the race
  where `func start` could otherwise win and find no `dist/main.js` yet.
- **React**: no new mechanism at all — `build:dev` aliases the existing per-environment
  `dev` build (adding `--sourcemap`, the one flag that build didn't already carry),
  `start` is Vite's own `preview` (serves `build`'s plain output statically, the
  closest Vite equivalent to "run what was compiled"), and `dev` is Vite's own
  inferred `dev` target (`serve` is documented-deprecated, removed in Nx 22).
- **Go**: no watch-and-rebuild loop exists in the toolchain the plugin ships (verified
  against `@nx-go/nx-go`'s `serve` executor schema: a one-shot `go run`), so `dev`
  shells out to `air`, the community tool. **Both `air`'s `build.cmd` and
  `@nx-go/nx-go:build`'s own `flags` array get shelled out via `execSync`**, so an
  unquoted `-gcflags=all=-N -l` is word-split on the embedded space and `go build`
  rejects the second half as an unknown flag — found by running it, not by reading
  either tool's docs. Fixed by quoting the value **inside** the flag string itself
  (`'-gcflags="all=-N -l"'`), which survives the shell join. `start` reads whichever
  single file landed in `dist/apps/<name>/` (the binary's name is platform-dependent)
  and runs it directly, replacing the previous `go run .` shape, which always
  recompiled and so never actually respected Nx's build cache.
- **Python**: no watch-and-rebuild tool ships with pip either — `watchmedo` (from the
  `watchdog` package, added to the workspace's already-fixed `requirements-dev.txt`)
  fills the same role `air` does for Go. **`watchmedo auto-restart` restarts on every
  subprocess exit by default, not just on a file change** — verified live: without
  `--no-restart-on-command-exit` it busy-loops restarting `main.py` the instant it
  finishes printing and exiting. The flag is what makes it a watch loop rather than a
  crash loop.
- **C#**: the cleanest of the six — `dotnet watch run` is native SDK tooling, needs no
  extra dependency, and already does exactly what `dev` asks. The one thing worth
  stating: bare `dotnet build`/`dotnet run` default to `Debug`, the *opposite*
  convention from the JS bundlers here (where a bare build means production), so
  `build`/`build:dev` are two explicit `-c Release`/`-c Debug` invocations rather than
  one bare command plus a flag.
- **Flutter**: `flutter run -d chrome` already **is** the uniform `dev` shape,
  unmodified — a debug build with native hot-reload, no extra tooling, for the same
  reason `dotnet watch run` needs none. The one thing that needed writing by hand was
  `build-dev` (`flutter build web --debug`), because it inherits a real, documented
  upstream bug the plugin's own `build` executor already works around
  (flutter/flutter#148542: a *relative* `--output` path breaks shader compilation,
  since flutter's `impellerc` subprocess resolves it against a different cwd than the
  parent) — the hand-written target resolves an absolute path with Node's own
  `path.join(process.cwd(), …)` before calling `flutter build web`, the same fix
  mirrored rather than reinvented. Flutter deliberately has **no `start` target**: the
  SDK ships no static file server for an already-built web bundle, and reaching for
  one (`dhttpd`, `python3 -m http.server`, …) was judged a worse trade than an honest,
  documented gap for a kind that already has a working `dev` command.

**Verified on every toolchain available in this sandbox** — real generation via the
built CLI, then real target execution, for Node, React/Vite, Go and Python — which is
what caught all three shell-quoting/race/restart-loop bugs above; none would have
surfaced from reading documentation or from the unit tests alone. C# and Flutter have
no live SDK in this environment, so their pieces are implemented and unit-tested but
**not independently verified against a real toolchain** — the next environment with
`dotnet`/`flutter` on PATH should run the e2e's `csharp`/`flutter` sections (already
extended with `build-dev`/`dev` assertions) as the first check.
