 ...
 * rather than JSON, enabling the unicorn, React (+ jsx-a11y) and TypeScript
 * built-in plugins. `correctness` is `error` (real bugs fail the build) and
 * `suspicious` is `warn`, so a freshly generated workspace still lints green.
 *
 * The rule opinions lean on StandardJS, but only the parts oxlint can express:
 * oxlint reimplements the ESLint core quality rules, while StandardJS's
 * *formatting* (no semicolons, single quotes, 2-space) is a formatter's job —
 * oxlint does not lint layout — so pair it with a formatter for that half.
 */
export const OXLINT_CONFIG = `import { defineConfig } from 'oxlint'

// Generated by MoNecromanCI. Quality rules (unicorn + React + TypeScript);
// StandardJS-inspired, but oxlint does not enforce formatting — layout
// (no semicolons, single quotes, 2-space) belongs to a formatter.
export default defineConfig({
  plugins: ['unicorn', 'react', 'typescript', 'import', 'oxc'],
  categories: {
    correctness: 'error',
    suspicious: 'warn',
  },
  rules: {
    // StandardJS quality opinions oxlint can express:
    eqeqeq: 'error',
    'no-var': 'error',
    'prefer-const': 'warn',
