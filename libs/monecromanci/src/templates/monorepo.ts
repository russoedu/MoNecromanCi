...
function packageJson (vars: MonorepoVars): string {
  return toJson({
    ...
    scripts:    {
      ...
      graph:              'nx graph',
      'nx:reset':         'nx reset',
      'pipeline:plan':    'node node_modules/monecromanci/dist/assets/build-templates/01-preparation.mjs',
      'pipeline:package': 'node node_modules/monecromanci/dist/assets/build-templates/03-package-apps.mjs --dry-run',
      release:            'nx release',
...
.pipeline-out/
.pipeline-staging/
# Ephemeral CI-run scratch state written by the shared engine (node_modules/monecromanci) — never vendored.
.build-templates/
*.log
.DS_Store
local.settings.json
...
\`build\` emits \`dist/\` and runs \`monecromanci\`'s own \`generate-dist-package.mjs\`
script, which writes a correct \`dist/package.json\`: it resolves real dependency versions from the **root**
 ...
 * `node_modules/monecromanci` instead of a vendored local copy.
 */
...
steps:
  ...
  - script: node node_modules/monecromanci/dist/assets/build-templates/01-preparation.mjs
    displayName: "[01] Resolve context, affected projects and execution plan"

  - script: node node_modules/monecromanci/dist/assets/build-templates/02-quality-control.mjs
  ...
  - script: node node_modules/monecromanci/dist/assets/build-templates/03-package-apps.mjs
  ...
  # re-checks these.
  - script: node node_modules/monecromanci/dist/assets/build-templates/04-publish-libs.mjs
  ...
  - task: AzureCLI@2
    ...
    inputs:
      ...
      inlineScript: node node_modules/monecromanci/dist/assets/build-templates/05-publish-documentation.mjs

  - script: node node_modules/monecromanci/dist/assets/build-templates/06-summary.mjs
 ...
 * `node_modules/monecromanci/dist/assets/build-templates/` (`monecromanci`
