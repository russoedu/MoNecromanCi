 ...
 * listed here: `eslint.config.mjs` is a thin re-export of
 * `monecromanci-toolchain/eslint.config.mjs`, and Node resolves that file's own
 * `import` statements from *inside* `node_modules/monecromanci-toolchain`'s own
 * dependency tree regardless of what the consuming repo declares — so those
 * plugins never need to be the consumer's own devDependencies. `eslint` itself
 * stays listed because it's invoked as a **binary** (`eslint . -c ...`), which
 * needs `node_modules/.bin/eslint` to exist at the consumer's own top level.
 */
...
function packageJson (vars: MonorepoVars): string {
  return toJson({
    ...
    scripts:    {
      ...
      graph:              'nx graph',
      'nx:reset':         'nx reset',
      'pipeline:plan':    'node node_modules/monecromanci-toolchain/build-templates/01-preparation.mjs',
      'pipeline:package': 'node node_modules/monecromanci-toolchain/build-templates/03-package-apps.mjs --dry-run',
      release:            'nx release',
...
function nxJson (vars: MonorepoVars): string {
  return toJson({
    ...
    targetDefaults: {
      ...
      'nx-release-publish': { dependsOn: ['build'], options: { packageRoot: '{projectRoot}/dist' } },
    },
    release: {
...
// Thin wrapper re-exporting the canonical config from the monecromanci-toolchain
// package (tsconfig.base.json, tsconfig.jest.json, jest.preset.mjs and
// typedoc.json are no longer vendored locally either — see
// 'monecromanci-toolchain/...' references throughout the per-project templates).
const eslintConfigWrapper = `// ESLint flat config — generated by MoNecromanCI. Re-sync with 'monecromanci doctor'.
export { default } from 'monecromanci-toolchain/eslint.config.mjs'
`
...
.pipeline-out/
.pipeline-staging/
# Ephemeral CI-run scratch state written by the shared engine (node_modules/monecromanci-toolchain) — never vendored.
.build-templates/
*.log
.DS_Store
local.settings.json
...
\`build\` emits \`dist/\` and runs \`monecromanci-toolchain\`'s
\`generate-dist-package.mjs\` script, which writes a correct \`dist/package.json\`: it resolves real dependency versions from the **root**
...
(\`${distPackageRootToken}/dist\` by default — where \`build\` emits), and natively skips
anything already on the registry. A project that packages itself from its root
instead (e.g. a bundled CLI with a \`files\` allow-list) overrides
\`packageRoot\` to \`${distPackageRootToken}\` in its own \`project.json\`. An unfiltered local
\`nx release publish\` builds first via the target's \`dependsOn: ['build']\`;
the CI publish step instead rebuilds the bumped projects explicitly, because
\`nx release publish --projects=…\` drops task dependencies and the build must
run after versioning anyway so the new version lands in each dist manifest.
 ...
 * works), each calling the shared engine scripts straight out of
 * `node_modules/monecromanci-toolchain` instead of a vendored local copy.
 */
...
# Generated by MoNecromanCI. Re-sync with 'monecromanci doctor'.
trigger:
  branches:
    include: ${branches}
  paths:
...
steps:
  ...
  - script: node node_modules/monecromanci-toolchain/build-templates/02-quality-control.mjs
  ...
  - script: node node_modules/monecromanci-toolchain/build-templates/03-package-apps.mjs
  ...
  # re-checks these.
  - script: node node_modules/monecromanci-toolchain/build-templates/04-publish-libs.mjs
  ...
  - task: AzureCLI@2
    ...
    inputs:
      ...
      inlineScript: node node_modules/monecromanci-toolchain/build-templates/05-publish-documentation.mjs

  - script: node node_modules/monecromanci-toolchain/build-templates/06-summary.mjs
...
/**
 * Workflow wrapper(s) for the selected provider(s) — Azure Pipelines and/or
 * GitHub Actions.
 *
 * @remarks
 * The shared engine scripts (`01-preparation.mjs`, etc.) are no longer
 * vendored into the repo: both providers call them straight out of
 * `node_modules/monecromanci-toolchain/build-templates/` (`monecromanci-toolchain`
 * is a devDependency of every generated repo).
 */
